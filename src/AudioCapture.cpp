...
float AudioCapture::leftVuDb() const {
  return leftVuDb_.load(std::memory_order_relaxed);
}

float AudioCapture::rightVuDb() const {
  return rightVuDb_.load(std::memory_order_relaxed);
}
...
QString AudioCapture::listDevicesString() {
  ...
  pa_context *ctx =
      pa_context_new(pa_mainloop_get_api(ml), "VU Meter Device List");
  if (!ctx) {
    pa_mainloop_free(ml);
  ...
  auto server_cb = [](pa_context *, const pa_server_info *info,
                      void *userdata) {
  ...
  auto sink_cb = [](pa_context *, const pa_sink_info *info, int eol,
                    void *userdata) {
    if (eol > 0 || !info)
      return;
    ...
    ctx->out->append(QString("Sink: %1%2\n")
                         .arg(QString::fromUtf8(info->name))
                         .arg(isDefault ? "   [DEFAULT]" : ""));
  ...
  auto source_cb = [](pa_context *, const pa_source_info *info, int eol,
                      void *userdata) {
    if (eol > 0 || !info)
      return;
    ...
    ctx->out->append(QString("Source: %1%2\n")
                         .arg(QString::fromUtf8(info->name))
                         .arg(isDefault ? "   [DEFAULT]" : ""));
    ctx->out->append(QString("  Description: %1\n\n")
                         .arg(QString::fromUtf8(info->description)));
  ...
  pa_operation *op2 =
      pa_context_get_source_info_list(ctx, source_cb, &sourceCtx);
...
void AudioCapture::stream_read_callback(pa_stream *s, size_t length,
                                        void *userdata) {
  ...
  const unsigned int samples =
      static_cast<unsigned int>(length / sizeof(float));
...
void AudioCapture::sink_info_callback(pa_context *c, const pa_sink_info *si,
  ...
  pa_proplist_sets(props, PA_PROP_FILTER_APPLY,
                   "echo-cancel noise-suppression=0 aec=0 agc=0");

  self->stream_ = pa_stream_new_with_proplist(
      self->context_, "VU Meter Capture", &nss, &si->channel_map, props);

  pa_proplist_free(props);
  pa_stream_set_state_callback(self->stream_,
                               &AudioCapture::stream_state_callback, self);
  ...
  pa_stream_connect_record(self->stream_, si->monitor_source_name, &attr,
                           PA_STREAM_ADJUST_LATENCY);
}

void AudioCapture::source_info_callback(pa_context *c, const pa_source_info *si,
                                        int is_last, void *userdata) {
  (void)c;
  auto *self = static_cast<AudioCapture *>(userdata);
  ...
  pa_stream_connect_record(self->stream_, si->name, &attr,
                           PA_STREAM_ADJUST_LATENCY);
}
...
void AudioCapture::context_state_callback(pa_context *c, void *userdata) {
  ...
  case PA_CONTEXT_READY: {
    ...
    if (self->options_.deviceType == 1) {
      // Source (mic)
      if (name) {
        op = pa_context_get_source_info_by_name(
            c, name, &AudioCapture::source_info_callback, self);
      } else {
        op = pa_context_get_source_info_list(
            c, &AudioCapture::source_info_callback, self);
      }
    } else {
      // Sink monitor (system output)
      if (name) {
        op = pa_context_get_sink_info_by_name(
            c, name, &AudioCapture::sink_info_callback, self);
      } else {
        op = pa_context_get_sink_info_list(c, &AudioCapture::sink_info_callback,
                                           self);
      }
    }
    if (op)
